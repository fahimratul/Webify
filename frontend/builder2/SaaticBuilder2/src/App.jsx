...
const App = () => {
  ...
  const publishToMarketplace = () => {
    ...
    // Cancel button
    container.querySelector('.publish-btn-cancel').addEventListener('click', () => {
      modal.close();
    });

    // Form submit
    container.querySelector('#publish-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      ...
      const description = container.querySelector('#pub-description').value.trim();
      const category = container.querySelector('#pub-category').value;
      const type = container.querySelector('#pub-type').value;
      const price = parseFloat(container.querySelector('#pub-price').value) || 0;
      ...
      if (type === 'paid' && (!price || price <= 0)) {
        statusEl.textContent = 'Please enter a valid price for paid templates.';
        statusEl.className = 'publish-status publish-status-error';
        return;
      }
      ...
      try {
        const res = await fetch('/api/marketplace/templates', {
          ...
          credentials: 'include',
          body: JSON.stringify({ title, description, html, css, type, price, category, tags }),
        });

        const data = await res.json();

        if (res.ok && data.success) {
          statusEl.textContent = 'Published successfully! Your template is now on the marketplace.';
          statusEl.className = 'publish-status publish-status-success';
          submitBtn.textContent = 'Published!';
          setTimeout(() => modal.close(), 2000);
        } else {
          statusEl.textContent = data.error || 'Failed to publish. Are you logged in?';
          statusEl.className = 'publish-status publish-status-error';
        ...
        statusEl.textContent = 'Network error. Make sure you are logged in and the server is running.';
        statusEl.className = 'publish-status publish-status-error';
        submitBtn.disabled = false;
        submitBtn.textContent = 'Publish to Marketplace';
      }
        ...
        <main class="editor-main">
          ...
          <header class="editor-topbar">
            ...
            <div class="topbar-right">
              ...
              <button class="btn-publish" onClick={publishToMarketplace} title="Upload to Marketplace">
